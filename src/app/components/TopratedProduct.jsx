...
const TopratedProduct = ({product}) => {
    ...
    const findTopRatedProduct = () => {
        const heightRate = allProduct.filter(item => item.rating.rate >= 4.0);
        return setTopRate(heightRate);
    } 
    useEffect(() => {
        findTopRatedProduct();
    },[])
                            ...
                            return (
                                ...
                                <div className='flex items-start justify-between flex-col topCard w-72 p-4 shadow-lg rounded-md overflow-hidden'>
                                        ...
                                        <div className='text-left mt-4 w-full'>
                                            ...
                                            <div>
                                                <button className='capitalize font-base cursor-pointer py-2 px-8 rounded-full w-full mt-4 bg-blue-600 hover:bg-green-600 hover:text-gray-50 transition-all duration-200 ease-in-out font-bold font-serif'>add to cart</button>
                                            </div>
                                        </div>
                                </div>
                                </Link>
